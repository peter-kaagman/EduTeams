...
Analog to the Groups API methods the Teams API gives you methos to add users. An important difference is that users are added to the team with a role. That way you are able to add an owner in one single transaction. These methos also impose the 20 users limit per transaction. But read on…
...
conversationMember: add - Microsoft Graph v1.0 | Microsoft Learn
